...
- 每次询问给出 $p, k$，求 $$ \max_{l \le p \le r}{\left( \sum_l^r{a_i} - k(\sum_l^r{b_i}) \right)} $$

首先对于包含 $p$ 这个要求进行转化，可以转化为 $$\max_{1 \le l \le r \le p} + \max_{p+1 \le l \le r \le n}$$

我们以式子左半边为例，即 $$ \max_{1 \le l \le r \le p}{\left( \sum_{i=l}^r{a_i} - k(\sum_{i=l}^r{b_i}) \right)} $$
